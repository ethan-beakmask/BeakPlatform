...
| 帳號 | 出廠兩個：`admin@sys-c0b7f64e345e`（SYSTEM_ADMIN，安裝密碼 `BpservFresh2026#Pw`，**仍 `must_change_password`、只用 curl 驗過登入 200**）與 `enterprise@sys-c0b7f64e345e`（ORG_ADMIN，is_original_admin，同密碼，**精靈未過**）。**這台從此由 Ethan 親自操作模擬讀者，AI 不要再對它跑 seed／改密／過精靈**（Ethan 2026-09-13 定調）。node展覽館 20 項已內建（分類 sc `7ojMqeuAKQJT_wFThMV_dQ`），示範帳號 `demo-staff／demo-manager／demo-director@sys-e5d0bc95fe43` 密碼同安裝密碼。2026-09-13 20:59 重裝前的帳號與密碼全部作廢 |
