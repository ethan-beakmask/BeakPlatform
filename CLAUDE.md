...
| 系統企業 | code `SYSTEM`，SYSTEM_ORG_CODE＝**`sys-c0b7f64e345e`**（2026-09-13 23:34 以 GitHub `85574843` 走 `INSTALL_DEMO=1` 全新重裝後的值，之前是 `sys-e5d0bc95fe43`；install.sh 隨機產生，**每次重裝都會變**，與 dev 的 `system.local` 不同。之前是 `sys-7936609f8530`、`sys-f692841b25dd`、`sys-1271967103b6`） |
| 帳號 | 出廠兩個：`admin@sys-c0b7f64e345e`（SYSTEM_ADMIN，安裝密碼 `BpservFresh2026#Pw`，**仍 `must_change_password`、只用 curl 驗過登入 200**）與 `enterprise@sys-c0b7f64e345e`（ORG_ADMIN，is_original_admin，**2026-09-14 01:02 已由 Claude 以 chrome-devtools 實走改密＋精靈：改密為 `BpservEnt2026#Wizard`、建成員 `sysops`（用戶編號留空→自動 0001）與管理員 `admin-sysops@sys-c0b7f64e345e`（同密碼），原始帳號已停用**；憑證 `/opt/tmp/verify/20260914-wizard-bpserv.log`）。**這台從此由 Ethan 親自操作模擬讀者，AI 不要再對它跑 seed／改密／過精靈**（Ethan 2026-09-13 定調）。node展覽館 20 項已內建（分類 sc `7ojMqeuAKQJT_wFThMV_dQ`），示範帳號 `demo-staff／demo-manager／demo-director@sys-e5d0bc95fe43` 密碼同安裝密碼。2026-09-13 20:59 重裝前的帳號與密碼全部作廢 |
| 示範企業 DemoSOC | **2026-09-13 23:34 起是 `install.sh` 的 `INSTALL_DEMO=1` 在全新安裝尾段自動建的**（`--demo` 首次在全新機器實走成功）：code `DEMOSOC`，domain `demo-soc.example`，sc `h0eMME7JBRCDD1YZrokJyM`，合約 `CTR-20260913-0001` 含五個模組，15 個帳號（含停用的原始 `admin@`），六個已發行流程。管理員 `admin-admin.ops@demo-soc.example`、示範帳號共用密碼**沿用 `ADMIN_INITIAL_PASSWORD`＝`BpservFresh2026#Pw`**（不是舊的 `DemoSoc-Staff2026#`）。防禦節點憑證 `ak_fb54061119cac611`＋`sa_ithome2026_waf_bd3215`，開通字串與 WAF 指令在 bpserv `/opt/BeakPlatform/demo-credentials.txt`（root 0600）與 `/opt/tmp/verify/20260913-bpserv-fresh-4-demo.log`。以下是更早（21:2x，手動 `seed_demo_org.py`）那份的記錄，識別碼已作廢：code `DEMOSOC`，domain `demo-soc.example`，五個模組合約，管理員 `admin-admin.ops@demo-soc.example`（已過精靈）＋13 位員工，密碼一律 `DemoSoc-Staff2026#`，六個已發行流程。以下是更早那份（重裝前）的記錄，識別碼已作廢：以下是重裝前那一份的記錄，僅供對照：code `DEMOSOC`，domain `demo-soc.example`，sc `9bz2WzH0QL3qHz7xPlCe1g`；合約含**全部五個模組**（Ethan 要抓圖，含 NoCode）；企業專屬庫 `org_2`。帳號密碼一律 `DemoSoc-Staff2026#`：管理員 `admin-admin.ops@demo-soc.example`（已過精靈，原始 `admin@demo-soc.example` 停用）、資安人員 `linda.hu`／`jason.ling`（SECURITY_STAFF）、資安主管 `kevin.ye`（SOC_SUPERVISOR）、差旅費申請鏈 `leo.heng → emma.feng → ryan.lan`。六個已發行流程（API Key 申請、代理指定、資安事件處置最小版／SOC 團隊版（sev>=3 路由已啟用）／單人版、差旅費人事取值示範）。防禦節點憑證 `ak_5269700274ff4aa9`＋`sa_defense_node_21_3b6c36`（開通字串在 `/opt/tmp/verify/20260913-pf272-bpserv-pairing.log`，SA 登入實測 200；**沒有節點在用**——`.13`／`.21` 仍指向 `.16` BELUGA）。重裝前的案件、EDL 設定、SMTP 設定組全部不在了；要寄信得重跑 `seed_smtp_test_config.py`（但它是同 DB 內複製，bpserv 系統企業現在沒有設定組，得手動建） |
| 登入 | 無 quick-login（dev 機限定），走 `POST /auth/login` JSON（`{"account":"admin@sys-c0b7f64e345e","password":"..."}`，成功回 200 + 改密 redirect，錯密碼 401）。**chrome-devtools 也走這條**：登入頁是三段驗證碼表單，不要填表單，先 `new_page` 開 `/auth/login`，再 `evaluate_script` 內 `fetch('/beakplatform/auth/login', {method:'POST', ...JSON})` 建 session，之後 `navigate_page` 到目標頁即已登入（2026-09-04 PF-239 實測） |
...
任一項失敗整個 bootstrap 非零退出；`SKIP_NODE_SHOWCASE=1` 可不種，但**它不是範例資料包**——
範例資料包只指 `seed_demo_org.py` 的示範企業，node展覽館 是系統預設企業出廠內建，讀者首次安裝就該有，
Ethan 2026-09-13 定調）。**2026-09-13 晚間起它是獨立安裝包**（`scripts/seed_node_showcase.py`）：
`--list` 印 20 項的層級／節點型別／已存在／會跑或跳過；`--org <code|domain|sc>` 指定企業（預設
`is_system_org` 那家）；`--apply [--only B8]` 追加或升級單項（冪等：依 code 找、bump revision 重發行）。
**系統級判定看授權不看身分**：`plan_showcase_items()` 對示範宣告的 `node_types` 逐一查
`is_node_allowed()`，受限節點（Os／Sys 六種）沒 grant 的企業自動 SKIP 並印原因，B16 另看 nocode 合約；
所以一般企業拿到的是企業級子集（dev DEMOSOC 實測 13 run／7 skip），`seed_demo_org.py --node-showcase`
可在建示範企業時一併佈建。密碼只在 B0 會跑時才必填（`--password` 或 `ADMIN_INITIAL_PASSWORD`）。
新增示範時要在 `SHOWCASE_ITEMS` 的 metadata 宣告 `node_types`，`test_node_showcase_package.py`
會拿原始碼比對（宣告了不存在的、或用了受限節點沒宣告的都紅）。五件猜不到的：
