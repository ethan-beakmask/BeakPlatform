...
用法：
  python3 cf_tunnel.py setup  --api-token <T> --hostname app.example.com [--tunnel-name integrated-waf]
  ...
  python3 cf_tunnel.py remove --api-token <T> --hostname old.example.com [--tunnel-name integrated-waf]
  python3 cf_tunnel.py status --api-token <T> --tunnel-name integrated-waf
  python3 cf_tunnel.py token  --api-token <T> --tunnel-name integrated-waf
...
def main(argv=None):
    ...
    def common(sp):
        ...
        sp.add_argument("--tunnel-name", default="integrated-waf", help="tunnel 名稱（預設 integrated-waf）")
