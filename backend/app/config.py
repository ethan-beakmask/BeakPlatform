...
class BaseConfig:
    ...
    # 本平台的 Cloudflare 路徑為：
    #   訪客 -> Cloudflare edge -> cloudflared -> nginx WAF-VM:8080 -> nginx :7000
    # 故 ProxyFix 還原出的 remote_addr 對 tunnel 流量恆為 WAF-VM，
    ...
    # PF-106: 防禦節點 ClickHouse（secstack）唯讀連線 -- 資安案件跨系統關聯串查。
    ...
    # OpenDefense od-bridge（防禦節點）唯讀查詢 -- 決策頁對帳 EDL / nftables 實際狀態。
    # bridge_client 連不上時回 None，呼叫端 fail-soft 顯示平台端記錄。
    OD_BRIDGE_URL = os.getenv('OD_BRIDGE_URL', '')
