...
function groupManager() {
    ...
    return {
        ...
        async init() {
            ...
            try {
                var res = await fetch(window.__BP + '/api/units/group-member-candidates?per_page=1000');
                if (res.ok) {
        ...
        async loadGroups() {
            try {
                var res = await fetch(window.__BP + '/api/units/groups?tree=true');
                var data = await res.json();
                if (res.ok) {
        ...
        async loadAllPeople() {
            ...
            var loadForGroup = async function(group) {
                try {
                    var res = await fetch(window.__BP + '/api/units/' + group.id + '/cross-members');
                    if (res.ok) {
                        var data = await res.json();
        ...
        async loadMembers(groupId) {
            try {
                var res = await fetch(window.__BP + '/api/units/' + groupId + '/cross-members');
                var data = await res.json();
                if (res.ok) {
        ...
        async updateGroup() {
            ...
            try {
                var res = await fetch(window.__BP + '/api/units/' + this.selectedGroup.id, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json', 'X-CSRFToken': getCsrfToken() },
                    body: JSON.stringify({
                        name: this.formData.name,
        ...
        async deleteGroup() {
            ...
            try {
                var checkRes = await fetch(window.__BP + '/api/units/' + this.selectedGroup.id + '?check_only=true&cascade=true', {
                    method: 'DELETE',
                ...
                var url = window.__BP + '/api/units/' + this.selectedGroup.id + '?cascade=true&confirm_members=true';
            ...
            try {
                ...
                if (['MANAGER', 'DEPUTY', 'PROXY1', 'PROXY2'].includes(roleType)) {
                    ...
                    if (currentHolder && currentHolder.user_secure_code !== userId) {
                        await fetch(window.__BP + '/api/units/' + this.selectedGroup.id + '/cross-members/' + currentHolder.id, {
                            method: 'PUT',
                            headers: { 'Content-Type': 'application/json', 'X-CSRFToken': getCsrfToken() },
                ...
                if (existing) {
                    // 已是成員：更新角色
                    var res = await fetch(window.__BP + '/api/units/' + this.selectedGroup.id + '/cross-members/' + existing.id, {
                        method: 'PUT',
                        headers: { 'Content-Type': 'application/json', 'X-CSRFToken': getCsrfToken() },
                    ...
                    // 新增成員
                    var res2 = await fetch(window.__BP + '/api/units/' + this.selectedGroup.id + '/cross-members', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json', 'X-CSRFToken': getCsrfToken() },
        ...
        async removeMember(leader, type) {
            ...
            try {
                var res = await fetch(window.__BP + '/api/units/' + this.selectedGroup.id + '/cross-members/' + leader.membershipId, {
                    method: 'DELETE',
                    headers: { 'X-CSRFToken': getCsrfToken() }
                });
                if (res.ok) {
        ...
        async removeMemberDirect(member) {
            ...
            try {
                var res = await fetch(window.__BP + '/api/units/' + this.selectedGroup.id + '/cross-members/' + member.id, {
                    method: 'DELETE',
                    headers: { 'X-CSRFToken': getCsrfToken() }
                });
                if (res.ok) {
        ...
        _attachExternalDrop: function() {
            ...
            container.addEventListener('drop', async function(e) {
                ...
                try {
                    var res = await fetch(window.__BP + '/api/units/' + targetId + '/cross-members', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json', 'X-CSRFToken': getCsrfToken() },
            ...
            var findAndUpdate = async function(items) {
                for (var i = 0; i < items.length; i++) {
                    if (items[i].id === groupId) {
                        var res = await fetch(window.__BP + '/api/units/' + items[i].id + '/cross-members');
                        if (res.ok) {
                            var data = await res.json();
