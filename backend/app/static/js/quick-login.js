...
function quickLoginManager() {
    return {
        ...
        async loadOrganizations() {
            try {
                const response = await fetch(window.__BP + '/dev/get-orgs');
        ...
        async selectOrg(secureCode) {
            ...
            try {
                const response = await fetch(`${window.__BP}/dev/get-users/${secureCode}`);
                const result = await response.json();
                if (result.success) {
                    // 排序：系統管理員 > 企業管理員 > 一般用戶
        ...
        async loginAs(user) {
            ...
            try {
                const response = await fetch(window.__BP + '/dev/quick-login', {
                    method: 'POST',
        ...
        async logout() {
            ...
            try {
                await fetch(window.__BP + '/dev/logout', { method: 'POST' });
