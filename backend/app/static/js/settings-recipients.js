...
function recipientGroupManager() {
    return {
        ...
        async loadGroups() {
            this.loading = true;
            try {
                var response = await fetch(window.__BP + '/api/admin/settings/recipient-groups');
        ...
        async loadOrgTree() {
            try {
                var response = await fetch(window.__BP + '/api/admin/settings/org-tree');
                var result = await response.json();
                if (result.success) {
        ...
        async searchUsers() {
            ...
            try {
                var response = await fetch(window.__BP + '/api/admin/settings/org-users?search=' + encodeURIComponent(this.userSearch));
                var result = await response.json();
                if (result.success) {
        ...
        async openEditModal(group) {
            this.isEditing = true;
            try {
                var response = await fetch(window.__BP + '/api/admin/settings/recipient-groups/' + group.id);
                var result = await response.json();
                if (result.success) {
        ...
        async saveGroup() {
            ...
            try {
                var url = this.isEditing
                    ? window.__BP + '/api/admin/settings/recipient-groups/' + this.formData.id
                    : window.__BP + '/api/admin/settings/recipient-groups';
        ...
        async previewRecipients(group) {
            ...
            try {
                var response = await fetch(window.__BP + '/api/admin/settings/recipient-groups/' + group.id + '/resolve');
                var result = await response.json();
                if (result.success) {
        ...
        async deleteGroup() {
            ...
            try {
                var response = await fetch(window.__BP + '/api/admin/settings/recipient-groups/' + this.groupToDelete.id, {
                    method: 'DELETE',
                    headers: {
                        'X-CSRFToken': getCsrfToken()
                    }
