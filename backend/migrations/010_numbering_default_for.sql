...
-- 支援 EMPLOYEE（企業成員編號預設）和 EXTERNAL（外部廠商預設）

-- 1. 新增 default_for 欄位
ALTER TABLE user_numbering_rules
ADD COLUMN IF NOT EXISTS default_for VARCHAR(20) NULL;

COMMENT ON COLUMN user_numbering_rules.default_for IS '預設用途: EMPLOYEE=企業成員預設, EXTERNAL=外部廠商預設, NULL=非預設';

-- 2. 遷移現有資料：原本 is_default=true 的設為 EMPLOYEE（企業成員預設）
