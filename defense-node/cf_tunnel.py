...
def upsert_ingress(token, acc, tunnel_id, hostname, service):
...
def cmd_setup(a):
    ...
    ingress = upsert_ingress(a.api_token, acc, tid, a.hostname, a.service)
    print(f"[+] ingress {a.hostname} -> {a.service}（共 {len(ingress)} 條，含 404 收尾）")
...
def cmd_status(a):
    ...
    for r in ((cfg.get("config") or {}).get("ingress") or []):
        print(f"  ingress {r.get('hostname','*')} -> {r.get('service')}")
    return 0
