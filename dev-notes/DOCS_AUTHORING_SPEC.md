...
### 先看這裡：兩個出口支援的語法不一樣（2026-08-17 實測）

**MkDocs 站吃 Material 的全套擴充；站內 `/help/` 只有
`extra` + `sane_lists` + `admonition` 三個**
（`backend/app/services/doc_catalog_service.py:647`）。

因此以下兩種寫法**在 MkDocs 站正常、在站內壞掉，而且不會報任何錯**：

| 寫法 | 站內的實際結果 |
|---|---|
| content tabs（`=== "標題"`） | `=== "標題"` 原樣印出，內容連 markdown 連結都不解析 |
| 縮排在 admonition 內的 fenced code | ` ```bash ` 原樣印出，指令擠成一行無法複製 |

`extra` 內含的 fenced_code 是 preprocessor，只認**行首**的 ```` ``` ````。
Material 用的 `pymdownx.superfences` 才處理縮排內的。

所以：**程式碼區塊一律寫在 admonition 外面**（提示卡只放步驟，指令接在卡後），
**不要用 content tabs**。要並列兩種做法就用兩個粗體小標或兩個 admonition。

（全站已於 2026-08-17 掃過一次，目前沒有任何一頁踩到這兩條。）
...
同一件事因編制/設定不同而有不同做法時，**放同一頁，不要拆成多份文件**。

~~content tabs（`=== "標題"`）~~ **站內不支援，不要用**（見本節開頭）。
改用兩個粗體小標，或兩個 `!!! note` 並列：
...
### 內容的讀者（2026-08-13 用戶交辦，見 BBN #5162）
...
**這一節管的是「寫多少、怎麼組織」，不是「用什麼語法」（語法看第三節）。**

### 9-1 少寫字，用圖定位；不要逐欄位描述畫面

用戶定調的兩個理由，決定了整套風格：

1. **寫太細，日後改程式又得改文件**——逐欄位、逐按鈕的描述是最快過期的內容
2. **現代讀者沒耐心讀大量文字**

因此：

- **一張圖讓讀者知道「在講哪個畫面」就夠了**，不必標註每個欄位
- 圖是**必要時**才放，不是每頁都要
- UI 本身是中文，操作對象多半知道自己要幹嘛；外語使用者有瀏覽器翻譯。
  **文件的價值在「順序」與「畫面上看不出來的規則」，不在複述欄位名**

### 9-2 圖的層級：第一層流程圖，不夠用才往下拆

```
完整 A 功能  →  第一層 SOP 流程圖（整個作業的節點順序）
                  └─ 某個節點講不清楚 → 該節點自己再一張流程圖
```

**先畫第一層，只有在某個節點真的講不清楚時才往下加一層。**
不要一開始就畫細，也不要把三層流程塞進同一張圖。

圖一律用 ```mermaid（第三節），兩個出口都渲染得出來。
...
在 md 裡寫成 admonition（兩個出口都認得，且視覺上會與內文區隔）：
...
- **`MENU:` 寫的是使用者在畫面上看到的選單路徑**，用全形斜線分隔，
  不是 URL、不是 `menu_items.code`
- **操作步驟只寫「點什麼、按什麼」**，一步一個動作。
  不要在步驟裡解釋原理——那寫在提示卡下方
- **提示卡看完就能做完的事，下面就不用再寫了**。
  只有真的需要詳細說明時，才在提示卡下方展開

### 9-4 頁面骨架

```
1. 提示卡（9-3）                    ← 大多數讀者只看這個
2. 必要時一張定位用的圖 / 流程圖      ← 9-1、9-2
3. 畫面上看不出來的規則              ← 文件的真正價值
4. 常見問題                          ← 只寫真的會被問到的
```

**第 3 項是判斷一頁「有沒有寫的必要」的標準**：
如果一個功能沒有任何畫面上看不出來的規則，提示卡就是全部，不必硬湊內文。

### 9-5 待補：站內 admonition 沒有樣式

`!!! abstract` 這類區塊在 MkDocs 站上有色條與圖示，
**站內 `/help/` 目前是純文字**——`platform-manual.css` 沒有 `.admonition` 樣式，
python-markdown 產出的 `<div class="admonition">` / `<p class="admonition-title">`
沒有任何規則接手。提示卡在站內因此看起來跟普通段落一樣。

要讓 9-3 的提示卡在站內發揮作用，這個樣式必須補上。
