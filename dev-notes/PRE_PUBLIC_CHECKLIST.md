...
git filter-repo \
  --path dev-notes/ --path tools/ --path devtools/ --path .forgejo/ \
...
`dev-notes/` 目前整目錄排除。公開前逐一審查，決定哪些文件可以放行：

- [ ] `dev-notes/BeakPlatform_Installation_Guide.md` -- 安裝指南 (已移除硬編碼密碼)
- [ ] `dev-notes/RATE_LIMITING.md` -- Rate Limiting 說明
- [ ] `dev-notes/DEVTOOLS_AND_POC.md` -- 開發工具說明
- [ ] 其他文件依內容逐一判斷
