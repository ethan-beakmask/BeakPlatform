...
**以下三項已由開發 session 執行完畢（2026-09-08 03:0x，憑證 `/opt/tmp/verify/20260908-pf251-bpserv.log`）**：

2. ~~**`push both`**~~ — **已完成**：`git push origin main`（`3d998fe3..39e74a62`）→ `bash scripts/push_github.sh`（過濾 33 項，GitHub `c4e5ca35`）：`git push origin main` → `bash scripts/push_github.sh`（過濾腳本要求工作區乾淨，現況已乾淨）。共 23 個 commit，含 PF-251 四期＋補丁 3a-1＋六個複審／設計 commit＋canary 文件收尾，以及三個與本案無關的既有 commit（企業管理頁 Tabs、CLAUDE.md 修訂、node 展覽館 PF-252）
3. ~~**bpserv 部署**~~ — **已完成**（`--update` 4/4 通過，版本 `c4e5ca35`）。**三處與原清單不同，照這裡的寫**：
   - **`provision_proxy_request_flow.py` 的 `--org` 只吃 `secure_code` 或 `domain_name`，不吃企業 code**：`--org DEMOSOC` 會失敗（腳本會列出可用企業），要寫 `--org demo-soc.example`
   - **節點定義不必手動種**：`OpProxyGrant` 由 `--update` 的 SQL extras（`seed_workflow_node_definitions.sql`，`ON CONFLICT (node_type) DO NOTHING`）自動帶入，部署後查得到
   - 遷移實際計數與原預期的差異見第 4 點
   - 實跑指令（bpserv 上的 Python 一律以 `beakplatform` 服務帳號執行並先載入 .env）：
     ```bash
     sudo bash /opt/BeakPlatform/scripts/install.sh --update
     sudo -u postgres psql -d beakplatform    # 七行 ALTER，見 scripts/migrate_proxy_assignments.py 檔頭
     sudo systemctl restart beakplatform
     cd /opt/BeakPlatform && sudo -u beakplatform bash -c \
       "set -a && source .env && set +a && venv/bin/python scripts/migrate_proxy_assignments.py --dry-run"
     # 確認計數後 --apply，再跑一次確認全 0
     cd /opt/BeakPlatform && sudo -u beakplatform bash -c \
       "set -a && source .env && set +a && venv/bin/python scripts/examples/provision_proxy_request_flow.py --org demo-soc.example --apply"
     ```
   - **系統企業（`sys-1271967103b6`）刻意未種出廠申請單**：它代表平台自己、沒有一般員工，且 SYSTEM_ADMIN 打模組 API 本來就常被 ACL 擋（PERM-04）。要種的話同一支腳本帶該 secure_code

<details><summary>原步驟（保留供對照）</summary>

   `--update` → **兩句 ALTER**（完整 SQL 在 `scripts/migrate_proxy_assignments.py` 的檔頭 docstring，共七行：`user_role_assignments` 五欄位＋部分索引、`fw_approval_records.acted_as_kind`）→
...
4. ~~**bpserv 驗收**~~ — **已完成，全項通過**（憑證同上，expect／got 逐項）。實際結果與原預期的三處差異：
   - `proxy_roles_deleted` 原寫 2、實際 **4**（DEMOSOC 與 SYSTEM 各 2 個 `DEPT_PROXY` 角色）
   - 另有 `roles_created=2`（兩企業補種 `DEPT_HEAD`）、`renamed=4`、`gate_snapshots=1`（DemoSOC 一個發行快照的關卡改指 `DEPT_HEAD`），原清單未列；`delegation_migrated`／`proxy_rows_migrated`／`menu_retired` 皆 0、`condition_retired` 1，與預期一致
   - `POST /api/my-proxy-assignments` 回 **405** 而非 404（該路徑仍有 GET 路由，只是沒有 POST 方法），效果等同不可用
   端到端另跑了一張完整的代理指定申請單：soc1 送單 → `OpFieldRead` 後 DYNAMIC 解析出 admin-soc1 → 同意 → 建 2 列 proxy（`flow:PROC-20260907-0001`）→ 結果寫回表單；驗完已軟刪那 2 列，申請單實例 `Form-260900004` 保留當樣本。
   **回歸基準 `OD-20260903-0002` 的 WAITING 任務部署前後一字不差（soc1 200／admin-soc1 403）。**

<details><summary>原驗收清單（保留供對照）</summary>

   （DemoSOC 沒有部門、沒有單位角色，所以重點不是角色@單位）：
...
**全案結案（2026-09-08 03:0x）**：dev 四期＋補丁 3a-1 全部完成並複審通過，已 push both，bpserv 部署與驗收通過。
PF-251 到此結束；後續若要做矩陣 #8 的 C 方案（`OpHrLookup` 多輸出角色@單位讓關卡改用 ROLE 規格）另開待辦。
