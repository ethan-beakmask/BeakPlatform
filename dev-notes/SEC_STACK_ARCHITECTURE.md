...
| `.20` 出站 | DROP 到 `.100` 與 `.10` |
| `.66`（2026-09-10 起） | 入站只放 `.10`、`.16`、`.20`→8000、ICMP；**出站 DROP 整個 192.168.0.0/24 加 `fe80::/10`、`fc00::/7`**（Internet 全開，DNS 改走 8.8.8.8／1.1.1.1）。IPv6 那兩條是 2026-09-11 補的：v4 的網段規則管不到 link-local，實測 `.66` 原本能經 `fe80::` 連到 `.16` 的 22／139／445（`.16` ip6tables 全 ACCEPT）。改前實測 `.66` 打得到路由器 `.1:80/443` 與 `.13:22/8123/9000`，改後內網零開放。憑證 `/opt/tmp/verify/20260910-dot66-lateral-check.log` |
