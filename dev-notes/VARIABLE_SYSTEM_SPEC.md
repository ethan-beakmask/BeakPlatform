...
- **不在這張表裡的鍵一律靜默回空字串**（`_resolve_fi()` 查不到就 `return ''`，
  不報錯、不留 log）。所以 `${fi.serial_number}`（誤把來源欄位名當變數名）這種寫法
  在 Telegram 告警、Email 內文裡的症狀是「單號：」後面空白，其餘照常送出——
  訊息看起來正常，只是少了關鍵資訊。**寫 `fi.` 變數前先對照本表**（2026-09-12 PF-260 踩過）
