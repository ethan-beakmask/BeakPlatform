...
| Q7 | 面板維持不關閉 |
| 2b 補問（21:50） | **開放員工自助建立代理授權，授權人限本人**——待辦 **PF-236**，**2026-09-03 22:30 已完成**（個人設定頁「我的代理授權」區塊＋ `/api/my-delegations`，實作記錄 `CALENDAR_SPEC.md` 六之四，憑證 `/opt/tmp/verify/20260903-pf236-my-delegations.log`） |

第 1 項已於 2026-09-03 完成（commit `e25f0bd8`），spec 暫存檔已隨主機非正常關機遺失，實作記錄以 `CALENDAR_SPEC.md` 六之三為準。

## 三、第三期四項（摘自 #5380，動工順序 **1 → PF-236 → 4 → B（時段級請假）→ 3 → 2**；1 與 PF-236 已完成）
