...
2. `dev-notes/integrations/open_defense_contract.md` — 對外契約 v1.0(已凍結,不要改)
3. `dev-notes/manifests/mod-open-defense.yaml` — open_defense 模組地圖
4. `dev-notes/open_defense_quickstart.md` — 4 個 UI 選單怎麼用(理解資料流)
5. `dev-notes/manifests/mod-form-workflow.yaml` — form_workflow 引擎結構(你會大量用它)
...
1. 跟用戶確認上述 5 個決策點
2. 認可後寫 manifest `dev-notes/manifests/mod-security-case-center.yaml` 並登錄 README.yaml
3. 從 SOC-1 開始一張一張做,每張完成 commit 並重啟 dev server 驗證
4. 完成後寫 `dev-notes/security_case_center_quickstart.md`(類比 open_defense_quickstart.md 風格)

---

## 不要做的事

- **不要改** `dev-notes/integrations/open_defense_contract.md`(對外契約凍結)
