...
## 6. 設計器（批次 2a 已完成，2b 待辦）

**批次 2a（grid，2026-08-05 完成）**：引擎下拉（流式／矩陣／自由）、矩陣編輯器
（復用 `grid-layout-editor.js` 的 `layoutOnly` 模式，列高單位改 px）、zone 屬性面板
（溢出處理、區塊內元件排序／移除、從未放置清單指派）、格子上顯示該 zone 的元件、
未放置元件的黃色警告（不阻擋儲存）。選「自由」會顯示尚未支援並阻擋儲存。

**踩過的坑（實作時務必保留這個行為）**：合併／重建矩陣會讓 zone 消失，
消失 zone 的元件必須有去處，否則靜默遺失且未放置清單也不會列回來。
