...
> 相關：`docs/SHARED_COMPONENTS_SPEC.md`（共用元件＝引用語意，與本檔的複製語意互補）、
> `docs/PAGE_IR_SPEC.md`（IR schema）、`docs/PAGE_IR_LAYOUT_ENGINES.md`（三個版面引擎）

## 0. 一句話定位

**樣板是複製語意**：以樣板建頁 ＝ 把樣板當下的 IR 複製一份成為新頁，此後兩者無關。
需要「改一次、所有頁面同步生效」的請用共用元件（引用語意），見
`docs/SHARED_COMPONENTS_SPEC.md`。
