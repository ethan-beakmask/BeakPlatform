...
稽核過程中確認**還有兩個超出當輪範圍的架構弱點**,這份是其中第二個(另一個見 `docs/handoff_authz_list_rbac.md`)。
...
2. `docs/manifests/security-core.yaml` — 確認 page_role_guard.py / auth_interceptor.py 是禁區
3. `docs/manifests/SECURITY_PITFALLS.md` — 第 3 點(雙鑰匙選單安全)
