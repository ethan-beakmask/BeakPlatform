...
事件處置已完成（Suricata 規則 disable + Vector intake_filter），詳見 `docs/security_incidents.md`。本任務是**強化 BeakPlatform 本身對爆量的抵抗力**。
...
完成後同步更新 `docs/manifests/mod-open-defense.yaml`，加入新增的 model（aggregation）、新增的 service method、新增的 migration。
...
然後可以開新對話處理 Guardian（見 `docs/handoff_beakwatch_guardian.md`）。
