...
- 所有企業的流程共用**同一份主機端認證**，平台會逐次記錄 AiAgent 節點用量，
  並依執行流程所屬企業歸屬到各企業。
- 企業可設定每日／每月的執行次數配額，以及每日／每月的**估算成本**配額。
  企業管理員可在「表單流程 → AI 用量與配額」頁面檢視當期用量、明細記錄，
  並調整企業層覆寫設定。
- 系統層預設值存放在 `system_settings` 的 `ai_node_defaults`。企業未覆寫的項目會沿用
  系統預設；若系統預設缺項，平台會使用內建保守預設。
- `cost_usd` 是 CLI 回報後依 Anthropic 標準 API 牌價換算的**等值估算成本**，
  不是實際扣款金額，也不代表訂閱制或其他方案的帳單金額。
