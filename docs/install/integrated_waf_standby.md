...
```bash
sudo bash /opt/integrated-waf/standby.sh release --service-ip 192.168.1.20
```
...
`failover.conf` 放在 `failover.sh` 旁邊（或 `/etc/integrated-waf/failover.conf`）：
...
INSTALL_DIR="/opt/integrated-waf"
LOG_DIR="/var/log/integrated-waf"  # 每次切換的完整輸出落地在這裡（failover-<日期>.log）
```
...
```bash
bash /opt/integrated-waf/failover.sh status          # 兩台誰持有服務 IP、cloudflared / od-bridge / blocklist
bash /opt/integrated-waf/failover.sh dry-run         # 只做前置檢查，不切換
bash /opt/integrated-waf/failover.sh switch          # 切到「現在沒持有服務 IP」的那台
bash /opt/integrated-waf/failover.sh to standby      # 切到指定節點（組態檔的名字、A/B、或 IP）
```
...
```bash
bash /opt/integrated-waf/failover.sh switch --yes
```
...
```bash
bash /opt/integrated-waf/failover.sh status
```
...
```bash
sudo bash /opt/integrated-waf/standby.sh takeover --service-ip 192.168.1.20
```
