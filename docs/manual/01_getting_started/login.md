...
!!! info "第一次登入這套系統？"
    如果貴公司的 BeakPlatform 是剛安裝完成、您拿到的是
    `admin@貴公司網域` 這組帳號，請先看安裝目錄裡的
    **`docs/install/first_login.html`**（用瀏覽器直接開，不需要登入）。

    第一次登入會被強制導向初始設定頁，該頁沒有選單也不能略過。
    那份文件說明為什麼要這樣做、要填什麼，以及完成後為什麼您會有兩組帳號。
...
!!! danger "救助關鍵字的用法"
    在救助欄位輸入含有救助關鍵字的字串，其餘照常填：密碼正確就**正常登入**，
    畫面與平常完全一樣、不會有任何提示；系統同時在背景寄警報信給貴公司全體管理員。
    這是受脅迫時求援用的，**不要拿來測試**，每觸發一次都會真的寄信。
    觸發後系統做什麼、為什麼這樣設計，見[登入救助機制](duress_login.md)。
...
- **畫面顯示「系統發信服務尚未就緒」或「寄送失敗」時，信不會來**，這是平台主機的發信設定問題，
  請聯絡系統管理員，不要一直重按（會觸發頻率限制）。若是在輸入驗證碼那一步看到寄送失敗，
  密碼並沒有被改動，同一組驗證碼在時限內可以再試一次
