...
- 起日填「今天」的合約當天就生效，判斷用的是**該企業設定的時區**的日曆日。伺服器時鐘是 UTC 也不影響（企業時區在該企業的「系統設定」頁）
...
| 狀態 | 意思 |
|---|---|
| 有效 | 狀態為有效，且今天在起訖日之間（「今天」依該企業設定的時區判斷，不是伺服器時鐘） |
| 尚未生效 | 開始日期還沒到 |
| 已過期 | 結束日期已過 |
| 停用 | 被手動停用，不可復原 |
...
集團沒有共用資料庫。這項功能曾經存在，已於 2026 年 9 月因安全考量移除；各企業的資料一律留在自己的專屬資料庫。
