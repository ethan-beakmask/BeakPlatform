...
這六頁是企業的人事結構資料，供管理員查詢與流程設計者對照。**表單流程不會自動讀取它們**：

- 簽核人由流程設計裡的簽核節點決定（發起人、指定成員、角色、部門、變數），
  不會自動送給職位設定裡填的「直屬主管」
- 金額分級要在流程的分支條件裡自己設定，系統不會拿核決權限的上限去比對表單金額

所以這裡設錯不會讓任何人多拿或少拿系統權限。反過來說，想讓「主管簽核」「金額分級」生效，
要到[流程設計](../04_form_workflow/workflows.md)裡設定。
