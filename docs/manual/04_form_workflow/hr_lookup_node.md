...
把某位成員在[職位設定](../03_org_setup/positions.md)裡的資料（職等、職稱、職系、部門、直屬主管、核決上限）
變成流程變數，讓後面的「分支」能依**職等高低、是不是管理職、哪個職系**分流，
...
- **直屬主管已停用或刪除時 `hr_direct_manager` 是空的**，簽核節點會找不到人。上線前先到[職位設定](../03_org_setup/positions.md)確認主管鏈
- **依金額找核決人是從直屬主管開始往上找**，不含申請人自己；每一站看那位主管職位的職等在所選類別的上限，
  第一個「上限 ≥ 金額」的人就是核決人。沒設上限的職等視為 0。整條鏈都不夠就 `hr_approver_found=false`
...
**簽核節點顯示找不到簽核人。**
依序查：取值節點有沒有放在簽核之前、簽核節點填的是變數名 `hr_direct_manager`（不是 `${...}`）、
申請人在職位設定裡有沒有有效職位、直屬主管是不是還在職。
