...
全程需要**企業管理員**權限；某一步的選單看不到，就是該帳號少了對應的選單授權，
到「角色管控 ／ 權限管理中心」處理，或請貴公司的平台管理員開通。
...
!!! danger "這一步不能用畫面新增，先看完再動作"
    處置中心認的是分類的**識別碼前綴**（`CAT_SECURITY_`），不是分類名稱。
    而畫面上的「新增父分類」拿到的是隨機識別碼——取名叫「資安案件」也沒用，
    用它建出來的案件在表單中心看得到，就是**不會出現在資安案件處置中心**，
    而且不會有任何錯誤訊息。

    所以這一步只有兩種正確做法。

**做法 A：沿用企業既有的資安分類**（多數企業開通時就有）

!!! abstract "作業：確認企業已有的資安分類"
    **MENU**：表單流程 ／ 分類管理

    1. 找名稱像「資安案件」的分類
    2. 有的話記下它，步驟二、三直接選它，本步驟就完成了

看不到這種分類，或不確定它的識別碼對不對，就走做法 B。

**做法 B：用建置腳本建**（腳本會產生正確前綴的識別碼）

見[送出第一筆事件](03_send_and_verify.md)的「一道指令建好整條鏈路」。
腳本會**先找既有的資安分類，找不到才建新的**，所以重複執行不會產生第二個分類；
建完可以回到分類管理確認它出現在清單上，再回來接著做步驟二。
