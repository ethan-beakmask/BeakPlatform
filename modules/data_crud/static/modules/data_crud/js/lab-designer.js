...
function labDesigner() {
    return {
        ...
        async savePage() {
            ...
            try {
                const res = await fetch('/api/data-crud/pages/' + this.pageSecureCode, {
                    method: 'PUT',
        ...
        async saveAsPage() {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    // 更新 URL
                    history.replaceState(null, '', '/data-crud/lab/' + this.pageSecureCode);
                    alert('已儲存');
                } else {
                    alert('儲存失敗: ' + (data.error || ''));
                }
        ...
        async openPage(secureCode) {
            ...
            history.replaceState(null, '', '/data-crud/lab/' + secureCode);
        },

        async _loadPage(secureCode) {
            try {
                const res = await fetch('/api/data-crud/pages/' + secureCode);
                const data = await res.json();
                if (!data.success) {
