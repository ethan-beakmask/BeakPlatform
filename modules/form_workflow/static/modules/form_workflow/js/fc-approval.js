...
function fcApproval() {
    return {
        ...
        async openApprovalModal(item) {
            ...
            try {
                // Step 1: 取得簽核鎖定
                const lockRes = await fetch(`${window.__BP}/api/form-center/pending-tasks/${item.queue_secure_code}/lock`, {
                    method: 'POST',
                ...
                if (data.success) {
                    ...
                    this._beforeUnloadHandler = () => {
                        fetch(`${window.__BP}/api/form-center/pending-tasks/${queueCode}/lock`, {
                            method: 'DELETE',
                            keepalive: true
                        }).catch(() => {});
                    };
        ...
        closeApprovalModal() {
            ...
            if (this.currentApproval?.queue_secure_code) {
                fetch(`${window.__BP}/api/form-center/pending-tasks/${this.currentApproval.queue_secure_code}/lock`, {
                    method: 'DELETE'
                }).catch(() => {});
            }
