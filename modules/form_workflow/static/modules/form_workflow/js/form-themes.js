...
function formThemeManager() {
    return {
        ...
        async loadThemes() {
            this.loading = true;
            try {
                const res = await fetch('/api/form-workflow/form-themes?all=1');
        ...
        async editTheme(theme) {
            // 取得含 CSS 的完整資料
            try {
                const res = await fetch(`/api/form-workflow/form-themes/${theme.secure_code}`);
                const json = await res.json();
                if (!json.success) {
                    this.showToast(json.message || '載入失敗', 'error');
                    return;
        ...
        async saveTheme() {
            ...
            try {
                ...
                if (this.editingTheme) {
                    url = `/api/form-workflow/form-themes/${this.editingTheme.secure_code}`;
                    method = 'PUT';
                } else {
                    url = '/api/form-workflow/form-themes';
                    method = 'POST';
                }
        ...
        async uploadCSS() {
            ...
            try {
                ...
                const res = await fetch('/api/form-workflow/form-themes/upload', {
                    method: 'POST',
                    body: formData
                });
                const json = await res.json();
        ...
        async deleteTheme() {
            ...
            try {
                const res = await fetch(`/api/form-workflow/form-themes/${this.deletingTheme.secure_code}`, {
                    method: 'DELETE'
                });
                const json = await res.json();
