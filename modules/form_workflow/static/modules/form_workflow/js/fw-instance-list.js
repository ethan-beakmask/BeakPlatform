...
function instanceListManager() {
    return {
        ...
        async loadInstances() {
            this.loading = true;
            try {
                var url = window.__BP + '/api/form-workflow/instances?page=' + this.pagination.page + '&tab=' + this.tab;
        ...
        async viewDetail(i) {
            try {
                var res = await fetch(window.__BP + '/api/form-workflow/instances/' + i.secure_code);
                var data = await res.json();
                if (data.success) {
