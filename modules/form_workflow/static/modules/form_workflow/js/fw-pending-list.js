...
function pendingListManager() {
    return {
        ...
        async loadTasks() {
            this.loading = true;
            try {
                var res = await fetch(window.__BP + '/api/form-workflow/pending-tasks');
        ...
        async openApproveModal(task) {
            ...
            try {
                var res = await fetch(window.__BP + '/api/form-workflow/pending-tasks/' + task.queue_secure_code);
                var data = await res.json();
                if (data.success) {
        ...
        async submitApproval() {
            ...
            try {
                var res = await fetch(window.__BP + '/api/form-workflow/pending-tasks/' + this.currentTask.queue_secure_code + '/approve', {
                    method: 'POST',
