...
class FormGridWidget {
    ...
    async _loadMasterViewConfig() {
        try {
            const res = await fetch('/api/nocode-builder/views/' + this.config.masterViewCode);
    ...
    async _loadDetailViewConfig() {
        try {
            const res = await fetch('/api/nocode-builder/views/' + this.config.detailViewCode);
            const data = await res.json();
            if (data.success) {
    ...
    async _loadMasterList() {
        ...
        try {
            let url = '/api/nocode-builder/views/' + this.config.masterViewCode + '/rows'
                + '?page=1&per_page=200';
    ...
    async _loadDetailRows(page) {
        ...
        try {
            let url = '/api/nocode-builder/views/' + this.config.detailViewCode + '/rows'
    ...
    async _selectMaster(rowId) {
        ...
        try {
            const res = await fetch(
                '/api/nocode-builder/views/' + this.config.masterViewCode + '/rows/' + rowId
            );
            const data = await res.json();
            if (data.success) {
    ...
    async _saveMasterEdit(saveBtn) {
        ...
        try {
            ...
            const res = await fetch(
                '/api/nocode-builder/views/' + this.config.masterViewCode
    ...
    async _openMasterModal(mode) {
        ...
        if (this.config.numberingRuleSc && pkeyCol) {
            try {
                const res = await fetch('/api/numbering/next?rule_sc=' + this.config.numberingRuleSc);
                const data = await res.json();
        ...
        saveBtn.addEventListener('click', async () => {
            ...
            if (this.config.numberingRuleSc && pkeyCol && data[pkeyCol] === suggestedPkey) {
                try {
                    await fetch('/api/numbering/consume', {
                        method: 'POST',
            ...
            try {
                ...
                const res = await fetch(
                    '/api/nocode-builder/views/' + this.config.masterViewCode + '/rows',
    ...
    async _openDetailModal(mode, rowId) {
        ...
        if (isEdit && rowId) {
            try {
                const res = await fetch(
                    '/api/nocode-builder/views/' + this.config.detailViewCode + '/rows/' + rowId
                );
                const data = await res.json();
        ...
        saveBtn.addEventListener('click', async () => {
            ...
            try {
                let url, method;
                if (isEdit) {
                    url = '/api/nocode-builder/views/' + this.config.detailViewCode + '/rows/' + rowId;
                    method = 'PUT';
                } else {
                    url = '/api/nocode-builder/views/' + this.config.detailViewCode + '/rows';
                    method = 'POST';
                }
