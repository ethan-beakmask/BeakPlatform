...
function rowFormManager() {
    ...
    return {
        ...
        async loadViewConfig() {
            try {
                const res = await fetch(`/api/nocode-builder/views/${this.secureCode}`);
                const data = await res.json();
                if (data.success) {
        ...
        async loadFormioSchema() {
            try {
                const res = await fetch(`/api/nocode-builder/views/${this.secureCode}/formio-schema`);
                const data = await res.json();
        ...
        async loadRowIntoFormio() {
            try {
                const res = await fetch(`/api/nocode-builder/views/${this.secureCode}/rows/${this.rowId}`);
                const data = await res.json();
        ...
        async save() {
            ...
            try {
                let url, method;
                if (this.rowId) {
                    url = `/api/nocode-builder/views/${this.secureCode}/rows/${this.rowId}`;
                    method = 'PUT';
                } else {
                    url = `/api/nocode-builder/views/${this.secureCode}/rows`;
                    method = 'POST';
                }
                ...
                if (data.success) {
                    ...
                    setTimeout(() => {
                        window.location.href = `/nocode-builder/views/${this.secureCode}`;
                    }, 800);
                } else {
