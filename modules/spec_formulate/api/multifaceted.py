...
@multifaceted_bp.route('/export/pdf', methods=['POST'])
@module_access_required('spec_formulate')
def export_pdf():
    """
    匯出多面向規格書為 PDF 文件。
...
@multifaceted_bp.route('/cg/info', methods=['GET'])
@module_access_required('spec_formulate')
def cg_info():
    """
    取得當前企業的集團共享 DB 資訊
...
@multifaceted_bp.route('/cg/tables', methods=['GET'])
@module_access_required('spec_formulate')
def cg_list_tables():
    """列出集團共享 DB 中的所有資料表"""
    from modules.spec_formulate.services.multifaceted.pg_table_manager import (
...
@multifaceted_bp.route('/cg/tables/<table_name>/introspect', methods=['GET'])
@module_access_required('spec_formulate')
