...
function dataSpecManager() {
    return {
        ...
        async loadData() {
            ...
            try {
                ...
                const [regRes, saRes] = await Promise.all([
                    fetch('/api/spec-formulate/specs/registry-overview'),
                    fetch('/api/spec-formulate/specs/standalone'),
                ]);
        ...
        async openNewSpec() {
            ...
            try {
                const res = await fetch('/api/spec-formulate/specs/available-templates');
        ...
        async createSpec(item) {
            ...
            try {
                const res = await fetch(
                    '/api/spec-formulate/specs/' + item.form_template_secure_code + '/sync-from-formio',
                    {
                        method: 'POST',
        ...
        async runCompare(item) {
            ...
            try {
                const res = await fetch(
                    '/api/spec-formulate/specs/' + item.form_template_secure_code + '/compare'
                );
                const data = await res.json();
                if (data.success) {
        ...
        // --- 獨立規格操作 ---
        goNewStandalone() {
            window.location.href = '/spec-formulate/new';
        },

        goEditStandalone(specSc) {
            window.location.href = '/spec-formulate/' + specSc + '/edit';
        },

        goSyncControl(ftSc) {
            window.location.href = '/spec-formulate/' + ftSc + '/sync';
        },
        ...
        async deleteStandalone(specSc) {
            ...
            try {
                const res = await fetch('/api/spec-formulate/specs/standalone/' + specSc, {
                    method: 'DELETE',
                    headers: { 'Content-Type': 'application/json' },
                });
                const data = await res.json();
