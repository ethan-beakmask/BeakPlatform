...
用途
    ...
    權威資料定義在 app.defaults.api_key_request_defaults，本腳本只負責 CLI。

用法
    cd /opt/BeakPlatform
    set -a && source .env && set +a
    venv/bin/python scripts/examples/provision_api_key_request_flow.py --org demo-soc.example --apply
...
from app.defaults.api_key_request_defaults import (  # noqa: E402
...
def main():
    ...
    from app.models import Organization, User, UserRoleAssignment
    ...
    with app.app_context():
        ...
        org_admin_role_sc = state.get('org_admin_role_secure_code')
        if not org_admin_role_sc:
            db.session.rollback()
            raise SystemExit('該企業缺少 ORG_ADMIN 角色，無法指定核准人')
        log(f'企業管理員角色：{org_admin_role_sc}')
        warn_if_no_org_admin_users(User, UserRoleAssignment, org_sc, org_admin_role_sc)

        if args.apply:
            result = seed_org_api_key_request_flow(org_sc, force=args.force)
