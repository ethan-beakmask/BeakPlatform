...
阻擋目標一律使用 123.123.0.0/16 這個 專案指定的安全測試網段
...
（範圍限定在 123.123.99.0/24，仍在 專案指定的安全測試網段內），
...
# 安全測試網段（專案指定）：阻擋目標一律落在這個範圍，不用任何真實或內網 IP。
SAFE_TEST_RANGE_NOTE = '123.123.0.0/16（專案指定的安全測試網段，非真實或內網位址）'
...
def build_decisionwriter_graph():
    nodes = [
        ...
        _node('node-Decision', 'DecisionWriter', '寫入封鎖決策', {
            ...
            '「該企業所有有效 block」不看 enforcement_points，而防禦節點的'
...
NT23_DESCRIPTION = (
    ...
    '本示範的預設值與允許範圍是 123.123.0.0/16（專案指定的安全測試'
    '網段），不是任何真實或內網位址。\n'
...
FORM_DECISIONWRITER_SCHEMA = {
    ...
    'components': [
        ...
        _hint('送出後先經過一個自簽的核准關卡，核准後由 Branch 判斷下面的'
              ...
              '使用 123.123.0.0/16（專案指定的安全測試網段），不會是'
              '任何真實或內網位址。'),
