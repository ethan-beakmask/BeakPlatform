...
用法
    ...
    venv/bin/python scripts/examples/provision_od_intake_for_org.py --org demo-soc.example --apply

參數
    --org <值>            必填。可填企業 secure_code 或 domain_name（例 demo-soc.example）
...
def parse_args(argv):
    ...
    parser.add_argument('--org', default=None,
                        help='必填。企業 secure_code 或 domain_name，例如 demo-soc.example')
...
def main(argv=None):
    ...
    if not args.org:
        raise SystemExit('缺少 --org。請指定企業 secure_code 或 domain_name，例如 --org demo-soc.example')
