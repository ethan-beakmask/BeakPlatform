...
參數說明
    --base-url   平台網址，預設 http://BeakPlatform-VM:7000/beakplatform
    --account    平台管理員帳號（必填）
    --password   平台管理員密碼（必填）
"""
...
DEFAULT_BASE = 'http://192.168.0.111:8000/beakplatform'
SUB_SYSTEM_NAME = '急難救助物資捐贈'
PORTAL_ROOT = Path('/opt/BeakPlatform/data/nocode_portals')
...
class Client:
    ...
    def __init__(self, base: str, account: str, password: str):
...
def main() -> int:
    ...
    ap.add_argument('--account', required=True, help='平台管理員帳號')
    ap.add_argument('--password', required=True, help='平台管理員密碼')
    ap.add_argument('--force', action='store_true', help='已有同名子系統時仍建立新的')
    args = ap.parse_args()

    api = Client(args.base_url, args.account, args.password)
