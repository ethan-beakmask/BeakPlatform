...
# 未指定 DB_PASS 時從 .env 的 DATABASE_URL 取密碼（安裝時隨機產生，沒有固定預設值）。
if [ -z "$DB_PASS" ] && [ -n "${DATABASE_URL:-}" ]; then
    DB_PASS=$(printf '%s' "$DATABASE_URL" | sed -n 's#^[a-z]*://[^:]*:\([^@]*\)@.*#\1#p')
fi
if [ -z "$DB_PASS" ]; then
    echo "找不到資料庫密碼：請設定 DB_PASS，或確認 .env 的 DATABASE_URL 含密碼。"
    exit 1
fi
